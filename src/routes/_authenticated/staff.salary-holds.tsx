...
<div className="flex flex-col gap-2 desktop:flex-row desktop:items-center">
    <div className="order-2 flex gap-2 overflow-x-auto pb-1 desktop:order-1 desktop:min-w-0 desktop:flex-1">{tabs.map(([key, label]) => <button key={key} onClick={() => { setTab(key); setSelectedIds(new Set()); }} className={`shrink-0 rounded-full px-3 py-2 text-xs font-medium ${tab === key ? "bg-primary text-primary-foreground" : "border bg-card"}`}>{label}{key !== "all" ? ` (${counts[key]})` : ""}</button>)}</div>
