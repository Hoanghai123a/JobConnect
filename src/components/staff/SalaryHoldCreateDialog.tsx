  ...
  return <Dialog open={open} onOpenChange={(value) => !saving && onOpenChange(value)}>
    <DialogContent className="max-h-[90dvh] overflow-y-auto rounded-2xl sm:max-w-lg">
      ...
      <div className="space-y-3">
        <div className="rounded-xl border bg-muted/30 p-3 text-sm"><div className="font-semibold">{history?.worker_name_snapshot || worker?.full_name || "—"}</div><div className="text-muted-foreground">{history?.expand?.factory?.name || worker?.company || "Chưa có công ty"}</div></div>
      ...
      <DialogFooter><Button variant="outline" onClick={() => onOpenChange(false)}>Đóng</Button><Button onClick={submit} disabled={saving}>{saving ? "Đang gửi..." : "Gửi yêu cầu"}</Button></DialogFooter>
    </DialogContent>
  </Dialog>;
}
