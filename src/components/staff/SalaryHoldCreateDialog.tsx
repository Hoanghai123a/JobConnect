...
export function SalaryHoldCreateDialog({
  ...
  const submit = async () => {
    ...
    if (history.recruiter_staff !== viewer.id)
      return toast.error("Bạn không phải người tuyển trong lịch sử gần nhất");
    setSaving(true);
    try {
        ...
        >
          <div className="rounded-xl border bg-muted/30 p-3 text-sm">
            <div className="font-semibold">
              {history?.worker_name_snapshot || worker?.full_name || "—"}
            </div>
            <div className="text-muted-foreground">
              {history?.expand?.factory?.name || "Chưa có lịch sử đi làm"}
            </div>
          </div>
