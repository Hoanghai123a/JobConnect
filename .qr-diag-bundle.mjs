...
// src/lib/cccd-qr.ts
var MAX_QR_SCAN_TIMEOUT_MS = 15e3;
var QUADRANT_OVERLAP_RATIO = 0.08;
var AUTO_LONG_EDGE = 1600;
var BASIC_LONG_EDGE = 1200;
var FULL_LONG_EDGE = 2e3;
var MAX_UPSCALE = 3;
...
function displayDateToPocketBase(value) {
  return normalizeDate(value);
}
function parseCccdQrText(text) {
  const normalizedText = text.split("\0").join("").replace(/[\u200B-\u200D\u2060\uFEFF]/g, "").replace(/[\r\n]+/g, "").trim();
...
function getStopReason(context) {
  if (context.signal?.aborted) return "cancelled";
  return Date.now() >= context.deadline ? "timeout" : null;
}
function recordAttempt(context, attempt) {
...
async function waitWithinDeadline(promise, context) {
...
function createHtmlImage(file) {
  const objectUrl = URL.createObjectURL(file);
  const image = new Image();
  const promise = new Promise((resolve, reject) => {
    image.onload = () => resolve(image);
    image.onerror = () => reject(new Error("Kh\xF4ng gi\u1EA3i m\xE3 \u0111\u01B0\u1EE3c \u1EA3nh"));
    image.src = objectUrl;
  });
  return {
    image,
...
async function decodeImage(file, context) {
  if (typeof createImageBitmap === "function") {
    ...
    if (bitmapResult.status === "failed") {
      void bitmapPromise.then((lateBitmap) => lateBitmap.close()).catch(() => void 0);
...
function buildQuadrantRegions(width, height) {
  const halfWidth = width / 2;
  const halfHeight = height / 2;
  const overlapX = width * QUADRANT_OVERLAP_RATIO / 2;
  const overlapY = height * QUADRANT_OVERLAP_RATIO / 2;
...
function renderScanRegion(image, region, targetLongEdge) {
...
function renderRotatedCanvas(source, rotation) {
...
function parseDetectedQrText(value) {
...
async function scanWithBarcodeDetector(detector, source) {
...
function scanWithJsQr(imageData) {
  const result = (0, import_jsqr.default)(imageData.data, imageData.width, imageData.height, {
    inversionAttempts: "attemptBoth"
  });
  return {
...
async function scanCanvas(detector, rendered, context) {
  const nativeResult = recordAttempt(
    context,
    await scanWithBarcodeDetector(detector, rendered.canvas)
  );
  if (nativeResult) return nativeResult;
  if (getStopReason(context)) return null;
  return recordAttempt(context, scanWithJsQr(getCanvasImageData(rendered)));
}
function enhanceQrImage(source) {
  const histogram = new Uint32Array(256);
  const sourcePixels = source.data;
  const pixelCount = source.width * source.height;
  for (let index = 0; index < sourcePixels.length; index += 4) {
    const luminance = Math.round(
      sourcePixels[index] * 0.299 + sourcePixels[index + 1] * 0.587 + sourcePixels[index + 2] * 0.114
    );
...
function thresholdQrImage(source) {
  const histogram = new Uint32Array(256);
...
function getDefaultTimeout(mode, mobile) {
  if (mode === "basic") return mobile ? 3e3 : 4e3;
  if (mode === "full") return mobile ? 1e4 : 12e3;
  return mobile ? 6e3 : 8e3;
}
function failed(reason) {
  return { status: "failed", reason };
}
async function scanCccdQrFromFileDetailed(file, options = {}) {
  ...
  try {
    ...
    reportProgress(options, { step: "whole", message: "\u0110ang ki\u1EC3m tra to\xE0n \u1EA3nh\u2026" });
    const originalNative = recordAttempt(
      context,
      await scanWithBarcodeDetector(detector, decoded.source)
    );
    ...
    const regionTarget = mode === "basic" ? BASIC_LONG_EDGE : mode === "full" ? FULL_LONG_EDGE : AUTO_LONG_EDGE;
    for (let index = 0; index < regions.length; index += 1) {
      ...
      reportProgress(options, {
        ...
        region: index + 1,
        total: 4,
        message: `\u0110ang ph\xF3ng to v\xF9ng ${index + 1}/4\u2026`
      });
    ...
    if (mode !== "basic") {
      for (let index = 0; index < regions.length; index += 1) {
        ...
        reportProgress(options, {
          ...
          region: index + 1,
          total: 4,
          message: `\u0110ang t\u0103ng \u0111\u1ED9 r\xF5 v\xF9ng ${index + 1}/4\u2026`
        });
        const rendered = renderScanRegion(decoded, regions[index], regionTarget);
        if (!rendered) continue;
        const enhanced = enhanceQrImage(getCanvasImageData(rendered));
        const enhancedResult = recordAttempt(context, scanWithJsQr(enhanced));
        if (enhancedResult) return { status: "success", data: enhancedResult };
        if (getStopReason(context)) continue;
        const thresholdResult = recordAttempt(context, scanWithJsQr(thresholdQrImage(enhanced)));
      ...
      for (let index = 0; index < regions.length; index += 1) {
        ...
        for (const rotation of rotations) {
          ...
          reportProgress(options, {
            ...
            total: 4,
            rotation,
            message: `\u0110ang xoay th\u1EED v\xF9ng ${index + 1}/4 (${rotation}\xB0)\u2026`
          });
...
async function scanCccdQrFromFile(file, options = {}) {
